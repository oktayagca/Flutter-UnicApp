...
class GroupChatViewModel with ChangeNotifier {
  ...
  void getMessageWithPagination(bool yeniMesajlarGetiriliyor) async {
    ...
    if (!yeniMesajlarGetiriliyor) state = GroupChatViewState.Busy;
  ...
  void newMessageListenerAssign() {
    ...
    _streamSubscription = _userRepository
        .getGroupMessages(currentUser.userID, chattedUser.userID)
        .listen((currentData) {
      if (currentData.isNotEmpty) {
        print("Listener tetiklendi ve son getirilen veri:" +
            currentData[0].toString());

        if (currentData[0].date != null) {
          if (_firstMessageInList == null) {
            _allMessage.insert(0, currentData[0]);
          } else if (_firstMessageInList.date.millisecondsSinceEpoch !=
              currentData[0].date.millisecondsSinceEpoch) {
            _allMessage.insert(0, currentData[0]);
          }
        }
        state = GroupChatViewState.Loaded;
